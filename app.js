...
app.use((req, res, next) => {
  // Allow requests from both 'https://snail-ide.com' and 'https://snail-ide.js.org,' this also allows our test frontend
  //res.header('Access-Control-Allow-Origin', 'https://snail-ide.com/', 'https://snail-ide.js.org', 'https://test-henna-alpha-94.vercel.app/');
...
app.get('/', async function (_, res) { // just basic stuff. returns the home page
    res.redirect('https://snail-ide.com/');
});
...
app.get('/api/users/loginLocal', async function (req, res) { // login with local account (like localhost)
    ...
    UserManager.verifyCode(privateCode).then(response => {
        ...
        const invalidRedirect =
            response.redirect !== 'https://snailshare.xyz/api/users/loginLocal'
    ...
    const body = JSON.stringify({
        ...
        embeds: [{
            ...
            author: {
                ...
                url: String("https://snail-ide.com/profile?user=" + String(bannedUser).substring(0, 50))
            },
    ...
    const body = JSON.stringify({
        ...
        embeds: [{
            ...
            author: {
                ...
                url: String("https://snail-ide.com/profile?user=" + String(reportedUser).substring(0, 50))
            },
            timestamp: new Date().toISOString()
        }]
    });
    ...
    const body = JSON.stringify({
        ...
        embeds: [{
            ...
            author: {
                ...
                url: String("https://snail-ide.com/profile?user=" + String(packet.username).substring(0, 50))
            },
            timestamp: new Date().toISOString()
        }]
    });
...
app.get('/api/projects/approve', async function (req, res) {
    ...
    {
        // post log
        const projectImage = String(`https://snailshare.xyz/api/pmWrapper/iconUrl?id=${project.id}&rn=${Math.round(Math.random() * 9999999)}`);
        const body = JSON.stringify({
            ...
            embeds: [{
                ...
                author: {
                    ...
                    url: String("https://snail-ide.com/profile?user=" + String(project.owner).substring(0, 50))
                },
                timestamp: new Date().toISOString()
            }]
        });
